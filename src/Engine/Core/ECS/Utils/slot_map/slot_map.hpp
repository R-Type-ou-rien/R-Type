...
struct handle_t {
    ...
    bool operator==(const handle_t& handle) const { return id == handle.id && generation == handle.generation; }

    bool operator!=(const handle_t& handle) const { return !(*this == handle); }

    static handle_t Null() { return {std::numeric_limits<uint32_t>::max(), 0}; }
};
...
class SlotMap {
   ...
   public:
    ...
    const std::vector<data_t>& get_data_list() const { return _dense; }
};
