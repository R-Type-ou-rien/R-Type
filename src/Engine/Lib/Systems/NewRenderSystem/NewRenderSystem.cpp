...
static void applyFlip(sf::Sprite& spr, bool flipX, bool flipY)
{
    ...
    spr.setOrigin({static_cast<float>(rect.size.x) * 0.5f,
        static_cast<float>(rect.size.y) * 0.5f});
                                    ...
                                    std::uint64_t& order)
{
    ...
    spr.setScale({transform.scale_x * spr.getScale().x,
                 transform.scale_y * spr.getScale().y});

    out.push_back(DrawCmd{
        static_cast<int>(spriteData.layer),
        order++,
        std::move(spr)
    });
}
