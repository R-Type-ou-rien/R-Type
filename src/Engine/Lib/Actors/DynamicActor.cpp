...
float DynamicActor::getMaxResourceStat(const std::string name) {
    ResourceComponent comp = _ecs.registry.getComponentConst<ResourceComponent>(_id);
...
std::pair<float, float> DynamicActor::getvelocity() {
    Velocity2D comp = _ecs.registry.getComponentConst<Velocity2D>(_id);
...
std::unordered_map<Action, ActionCallback> DynamicActor::getActionCallbackOnPressed() {
    ActionScript comp = _ecs.registry.getComponentConst<ActionScript>(_id);
...
std::vector<std::pair<float, float>> DynamicActor::getPattern()
{
    ...
    PatternComponent comp = _ecs.registry.getComponentConst<PatternComponent>(_id);
