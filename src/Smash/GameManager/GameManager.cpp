...
void GameManager::init(ECS& ecs) {
    ...
    ecs.registry.addComponent<TextComponent>(_UiPercentPlayer1, TextComponent{"0%", "src/Smash/Content/font.otf", 40, sf::Color::Blue, 200, 700});
    ecs.registry.addComponent<TextComponent>(_UiPercentPlayer2, TextComponent{"0%", "src/Smash/Content/font.otf", 40, sf::Color::Red, 800, 700});

    _UiLivesPlayer1 = ecs.registry.createEntity();
    _UiLivesPlayer2 = ecs.registry.createEntity();
    ecs.registry.addComponent<TextComponent>(_UiLivesPlayer1, TextComponent{"3", "src/Smash/Content/font.otf", 20, sf::Color::Blue, 200, 750});
    ecs.registry.addComponent<TextComponent>(_UiLivesPlayer2, TextComponent{"3", "src/Smash/Content/font.otf", 20, sf::Color::Red, 800, 750});
    return;
}
...
void GameManager::handleSpecialAttack(ECS& ecs, Player& attacker, const std::string& input) {
    if (ecs.input.isJustPressed(input) && attacker.canShoot()) {
        ...
        ecs.registry.addComponent<SmashProjectileComponent>(projectile, {attacker.getId(), 3.0f, 2.0f}); // 3 dmg, 2s lifetime
...
void GameManager::updateProjectiles(ECS& ecs, float dt) {
    ...
    for (size_t i = 0; i < projectiles.size(); ++i) {
        ...
        if (!ecs.registry.hasComponent<SmashProjectileComponent>(entityId) || 
            !ecs.registry.hasComponent<transform_component_s>(entityId)) continue;
...
void GameManager::handleCombat(ECS& ecs, Player& attacker, Player& victim, const std::string& attackInput, const std::string& attackName, int damage, float forceBase, float forceScaling) {
    if (ecs.input.isJustPressed(attackInput)) {
        ...
        bool collision = (std::abs(posAttacker.x - posVictim.x) < 50.0f) && (std::abs(posAttacker.y - posVictim.y) < 50.0f);
