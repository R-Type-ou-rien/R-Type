...
void GameManager::init(Environment& env, InputManager& inputs) {
    ...
    _player = env.spawn<Player>(SpawnPolicy::PREDICTED, std::pair<float, float>{100.f, 300.f});
...
void GameManager::update(Environment& env, InputManager& inputs) {
    if (_player) {
        ...
        if (env.getECS().registry.hasComponent<HealthComponent>(player_id)) {
            int hp = _player->getCurrentHealth();
            if (env.getECS().registry.hasComponent<TextComponent>(_uiEntity)) {
                auto& text = env.getECS().registry.getComponent<TextComponent>(_uiEntity);
