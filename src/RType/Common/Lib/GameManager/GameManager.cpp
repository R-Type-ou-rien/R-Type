...
void GameManager::init(Environment& env, InputManager& inputs) {
    ...
    try {
        ...
        if (!_current_level_config.game_config.empty()) {
            _game_config = ConfigLoader::loadGameConfig(_current_level_config.game_config,
                                                        ConfigLoader::getRequiredGameFields());
        }
    } catch (const std::exception& e) {
...
void GameManager::loadNextLevel(Environment& env) {
    ...
    for (auto entity : all_entities) {
        ...
        for (const auto& tag : tags.tags) {
            ...
            if (tag == "ENEMY" || tag == "BOSS") is_enemy = true;
            if (tag == "PROJECTILE") is_projectile = true;
        }
        
        if (!should_keep) {
            entities_to_destroy.push_back(entity);
            if (is_player) player_count++;
            else if (is_enemy) enemy_count++;
            else if (is_projectile) projectile_count++;
            else other_count++;
        }
    }
    ...
    std::cout << "[GameManager] Destroying " << untagged_health_entities.size() << " untagged health entities (potential invisible enemies)" << std::endl;
    for (auto entity : untagged_health_entities) {
        ecs.registry.destroyEntity(entity);
    }
    
    size_t total_cleaned = entities_to_destroy.size() + timers.size() + scripted_spawners.size() + pod_spawners.size() + untagged_health_entities.size();
    ...
    std::cout << "[GameManager] ===== LEVEL " << (_current_level_index + 1) << "/" << _level_files.size() << " LOADED SUCCESSFULLY! =====" << std::endl;
}
