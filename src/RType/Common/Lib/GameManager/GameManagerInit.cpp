...
void GameManager::initPlayer(Environment& env) {
    ...
    if (_player) {
        ...
        _player->setTextureDimension(rect{
            static_cast<float>(_player_config.sprite_x.value()), static_cast<float>(_player_config.sprite_y.value()),
    ...
    if (!env.isServer()) {
        ...
        ecs.registry.addComponent<TextComponent>(
            _uiEntity, {"HP: " + std::to_string(_player_config.hp.value()),
                        "content/open_dyslexic/OpenDyslexic-Regular.otf", 28, sf::Color::White, 10, 10});

        // Score UI
        _scoreEntity = ecs.registry.createEntity();
        ecs.registry.addComponent<TextComponent>(
            _scoreEntity,
            {"Score: 0", "content/open_dyslexic/OpenDyslexic-Regular.otf", 28, sf::Color::Yellow, 10, 50});
