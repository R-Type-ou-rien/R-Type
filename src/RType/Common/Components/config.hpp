...
class ConfigLoader {
public:
    static EntityConfig loadEntityConfig(const std::string& filepath, 
    ...
    static std::map<std::string, EntityConfig> loadEnemiesConfig(const std::string& filepath,
        ...
        for (const auto& [section, found_fields] : found_fields_per_section) {
            ...
            if (!missing_fields.empty()) {
                throw std::runtime_error("Missing required fields in [" + section + "] section of " 
    ...
    static GameConfig loadGameConfig(const std::string& filepath,
                                       const std::set<std::string>& required_fields) {
        GameConfig config;
        std::ifstream file(filepath);
        if (!file.is_open()) {
        ...
        while (std::getline(file, line)) {
            ...
            found_fields.insert(key);
            if (key == "boss_spawn_time") config.boss_spawn_time = std::stof(value);
            else if (key == "wave_interval") config.wave_interval = std::stof(value);
            else if (key == "min_enemies_per_wave") config.min_enemies_per_wave = std::stoi(value);
            else if (key == "max_enemies_per_wave") config.max_enemies_per_wave = std::stoi(value);
            else if (key == "obstacle_start_time") config.obstacle_start_time = std::stof(value);
            else if (key == "obstacle_end_time") config.obstacle_end_time = std::stof(value);
            else if (key == "boss_intro_delay") config.boss_intro_delay = std::stof(value);
        }
...
private:
    static void parseEntityValue(EntityConfig& config, const std::string& key, const std::string& value) {
        if (key == "hp" || key == "max_hp") config.hp = std::stoi(value);
        else if (key == "damage") config.damage = std::stoi(value);
        else if (key == "speed") config.speed = std::stof(value);
        else if (key == "fire_rate") config.fire_rate = std::stof(value);
        else if (key == "can_shoot") config.can_shoot = (value == "true");
        else if (key == "shoot_at_player") config.shoot_at_player = (value == "true");
        else if (key == "follow_player") config.follow_player = (value == "true");
        else if (key == "pattern") config.pattern = value;
        else if (key == "amplitude") config.amplitude = std::stof(value);
        else if (key == "frequency") config.frequency = std::stof(value);
        else if (key == "sprite") config.sprite_path = value;
        else if (key == "sprite_x") config.sprite_x = std::stoi(value);
        else if (key == "sprite_y") config.sprite_y = std::stoi(value);
        else if (key == "sprite_w") config.sprite_w = std::stoi(value);
        else if (key == "sprite_h") config.sprite_h = std::stoi(value);
        else if (key == "score_value") config.score_value = std::stoi(value);
        else if (key == "start_x") config.start_x = std::stof(value);
        else if (key == "start_y") config.start_y = std::stof(value);
        else if (key == "projectile_damage") config.damage = std::stoi(value);
        else if (key == "scale") config.scale = std::stof(value);
    }
    
public:
    static const std::set<std::string>& getRequiredPlayerFields() {
        static const std::set<std::string> fields = {
            "hp", "damage", "speed", "fire_rate", "sprite", 
