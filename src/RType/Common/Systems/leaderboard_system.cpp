...
void LeaderboardSystem::update(Registry& registry, system_context context) {
  ...
  auto destroy_leaderboard_ui = [&registry]() {
    ...
    for (auto e : tagged) {
      if (!registry.hasComponent<TagComponent>(e)) continue;
      ...
      if (should_destroy) to_destroy.push_back(e);
    }
  ...
  for (auto entity : entities) {
    ...
    if (leaderboard.victory && leaderboard.elapsed_time >= leaderboard.auto_hide_duration) {
      std::cout << "[LeaderboardSystem] Auto-hiding leaderboard after " << leaderboard.auto_hide_duration << " seconds" << std::endl;
    ...
    if (leaderboard.is_displayed && !leaderboard.ui_created) {
      ...
      if (leaderboard.victory) {
        ...
        registry.addComponent<TextComponent>(timerEntity, {timerText, "src/RType/Common/content/open_dyslexic/OpenDyslexic-Regular.otf", 48, sf::Color::Yellow, 1400.0f, 900.0f});
      }
      ...
      for (auto timer_entity : timer_entities) {
        if (!registry.hasComponent<TagComponent>(timer_entity)) continue;
